...
function DamageNumber:draw(x, y)
    ...
    label_draw(
        self.text, self.opt, _x + x, _y + y, w, h, self.scale * 0.5, self.scale * 0.5
    )
end
...
DamageNumberServer.remap[mechanics.damage.transform.damage] = function(
        self, state, info, user, target, damage
)
    local scene_graph = self:root()
    local body = scene_graph:get_body(target)
    local p = body:get_shape():center()
    self:damage(p, info)
end
...
DamageNumberServer.remap["combat.damage:true_damage"] = function(
        self, state, info, args, root
)
    local sprite = get_sprite(root, args.target)
    local n = root:child()
