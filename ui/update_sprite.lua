...
function Sprite:death()
    ...
    cs.death = color(1, 1, 1, 1)

    local function animate_death()
        local t = tween(0.2, cs.death, color(1, 0.2, 0.1, 1))
            :call(function() self:update_color() end)
        event:wait(t, "finish")
        local t = tween(0.2, cs.death, color(0, 0, 0, 0))
            :call(function() self:update_color() end)
        event:wait(t, "finish")
    end
...
function Sprite:frozen(active)
    if active then
        self:get_color_stack().freeze = color(0.2, 0.3, 1)
...
function Sprite:oil(active)
    if active then
        self:get_color_stack().oil = color(0.1, 0.1, 0.1)
...
function Sprite:wet(active)
    if active then
        self:get_color_stack().wet = color(0.2, 0.7, 0.9)
...
function Sprite:bleed(active)
    if active then
        self:get_color_stack().bleed = color(0.9, 0.1, 0.1)
...
function Sprite:alive()
    local cs = self:get_color_stack()
    cs.death = cs.death or color(1, 1, 1, 1)
    tween(0.4, cs.death, color(1, 1, 1, 1))
        :call(function() self:update_color() end)
end
