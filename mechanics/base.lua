local buff = require "mechanics.buff"
...
function mech.set_health(state, args)
    local id = args.id
    state = state:write(join("actor/health", id), args.health)
    if args.max_health then
        state = state:write(join("actor/max_health", id), args.max_health)
    end
    return state
end
